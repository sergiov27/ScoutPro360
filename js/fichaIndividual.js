...
const basesDatos = {
    ...
    escocia: {
        database: "escocia_database.json",
        percentiles: "escocia_percentiles.json"
    },

    inglaterra: {
        database: "inglaterra_database.json",
        percentiles: "inglaterra_percentiles.json"
    },

    portugal: {
        database: "portugal_database.json",
        percentiles: "portugal_percentiles.json"
    },

    san_marino: {
        database: "san_marino_database.json",
        percentiles: "san_marino_percentiles.json"
    },

    holanda: {
        database: "holanda_database.json",
        percentiles: "holanda_percentiles.json"
    }, 

    espana: {
        database: "espana_database.json",
        percentiles: "espana_percentiles.json"
    }        
};
...
const metricasCreacion = [
    ...
    { nombre: "Asistencias p90", claves: [
        "percentile_asist_remate_90",
        "percentile_asistencias_90"
    ]},

    { nombre: "Participación en Construcción", claves: [
        "percentile_second_assists_90",
        "percentile_third_assists_90"
    ]},
...
function actualizarBloqueMetricas(contenedor, metricas, jugadorPercentiles) {
    ...
    metricas.forEach(metrica => {
        ...
        fila.className = "filaMetrica";

        const nombre = document.createElement("div");
        nombre.className = "filaMetricaNombre";
        nombre.textContent = metrica.nombre;

        const valor = document.createElement("div");
        valor.className = "filaMetricaValor";
        ...
        valor.style.color = obtenerColorBarraPercentil(percentil);
	...
	function poblarSelectorJugadores() {
		...
		if (!jugadoresLiga.length) {
			...
			if (!selectorJugador.value && jugadoresOrdenados.length) {
				selectorJugador.value = jugadoresOrdenados[0].jugador;
			}

			actualizarFichaJugador(selectorJugador.value || jugadoresOrdenados[0]?.jugador || "");
		}
